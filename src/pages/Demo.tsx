...
import {
  ...
  ArrowRight, Loader2, Repeat, ShieldCheck, Sliders, Wand2, ChevronDown,
} from "lucide-react";
...
const Demo = () => {
  ...
  const [selectedMood, setSelectedMood] = useState<string | null>("Relax"); // pre-selezionato
      ...
      <section className="px-4 py-10 md:py-20">
        <div className="container max-w-3xl space-y-6">
          ...
          <div className="text-center">
            ...
            <h1 className="mt-4 text-2xl font-bold tracking-tight text-foreground md:mt-6 md:text-4xl">
              In pochi secondi <span className="gradient-primary-text">organizziamo la tua settimana</span>
            </h1>
            <p className="mt-2 text-sm text-muted-foreground md:text-base">
              Pasti e lista della spesa, già pronti. Puoi modificare tutto dopo.
            </p>
          </div>

          {/* QUICK START — MOOD (sezione più prominente) */}
          <div className="rounded-2xl border-2 border-accent/30 bg-gradient-to-br from-accent/5 via-card to-card p-5 md:p-8 shadow-lg shadow-accent/10">
            <div className="flex items-center gap-2 mb-1">
              <Wand2 className="h-5 w-5 text-accent" />
              <span className="text-xs font-semibold uppercase tracking-wider text-accent">Inizia da qui</span>
            </div>
            <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">
              Come ti vuoi sentire questa settimana?
            </h2>
            <p className="text-sm text-muted-foreground mb-5">
              Scegli un mood. Pensiamo a tutto noi.
            </p>

            <div className="flex flex-wrap gap-2 md:gap-3 mb-5">
              {moods.map((m) => (
                <button key={m.label} onClick={() => handleMoodSelect(m.label)}
                  className={`flex items-center gap-2 rounded-full border-2 px-4 py-2.5 md:px-5 md:py-3 text-sm font-medium transition-all duration-200 hover:scale-105 ${
                    selectedMood === m.label ? m.colorClass + " shadow-md scale-105" : "border-border bg-card hover:border-muted-foreground/20"
                  }`}>
            ...
            {/* Tempo — quick start essenziale */}
            <div className="mb-5">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 block">Quanto tempo hai?</label>
              <div className="flex gap-2">
                {(["rapido", "elaborato"] as const).map((t) => (
                  <button key={t} onClick={() => setTimeMode(t)}
                    className={`flex-1 rounded-xl border-2 py-3 text-xs md:text-sm font-medium transition-all duration-200 ${
                    ...
                    }`}>{t === "rapido" ? "⚡ Veloce (< 20 min)" : "👨‍🍳 Tempo libero (> 30 min)"}</button>
                ))}
              </div>
            </div>

            {/* CTA PRIMARIA — visivamente dominante */}
            <Button
              size="lg"
              onClick={handleOrganize}
              disabled={!selectedMood || isGenerating}
              className="w-full rounded-2xl bg-accent py-7 text-base md:text-lg font-bold text-accent-foreground shadow-xl shadow-accent/30 transition-all duration-200 hover:scale-[1.02] hover:shadow-2xl hover:shadow-accent/40 disabled:opacity-50"
            >
              {isGenerating ? (
                <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Sto organizzando…</>
              ) : (
                <>
            ...
            <p className="mt-2 text-center text-xs text-muted-foreground">
              Creiamo automaticamente pasti e lista della spesa per te.
            </p>

            {isGenerating && (
              <div className="mt-6 flex flex-col items-center gap-2 animate-fade-in">
                <p className="text-sm font-medium text-accent">
                  L'AI sta creando il tuo piano di 21 pasti…
                </p>
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  <ShieldCheck className="h-3 w-3" />Vincoli applicati: allergie → dieta → budget → mood
                </p>
              </div>
            )}
            ...
            {showResults && currentMenu && (
              <div className="mt-6 animate-fade-in">
                <p className="mb-1 text-sm font-medium text-primary flex items-center gap-1">
                  ...
                  Ecco il tuo piano — Mood: {selectedMood}
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {currentMenu.map((d) => {
                    ...
                    return (
                      <div key={d.day} className="rounded-xl border border-border/60 bg-card p-3 md:p-4 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
                ...
                <p className="mt-3 text-xs text-muted-foreground text-center">
                  Anteprima 3 giorni · Il piano completo (21 pasti) ti aspetta nella dashboard
                </p>
          ...
          {/* OPTIONAL — collassato di default */}
          <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
            <button
              onClick={() => setShowOptional(!showOptional)}
              className="w-full flex items-center justify-between gap-3 p-4 md:p-5 hover:bg-secondary/30 transition-colors"
            >
              <div className="flex items-center gap-3 text-left">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">Personalizza (facoltativo)</p>
                  <p className="text-xs text-muted-foreground">Dieta, allergie, budget, dispensa…</p>
                </div>
              </div>
              <ChevronDown className={`h-5 w-5 text-muted-foreground transition-transform ${showOptional ? "rotate-180" : ""}`} />
            </button>

            {showOptional && (
              <div className="border-t border-border/60 p-4 md:p-6 space-y-6 animate-fade-in">
                <div>
                ...
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">
                    Quanto pesa il mood: <span className="text-accent">{Math.round(moodWeight[0] * 100)}%</span>
                  </label>
                ...
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">Occasione (opzionale)</label>
                  <div className="flex flex-wrap gap-2">
                    {occasions.map((o) => (
                      <button key={o.label} onClick={() => setSelectedOccasion(o.label === selectedOccasion ? null : o.label)}
                ...
                <div className="flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-secondary/20 p-4">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-9 w-9 items-center justify-center rounded-full transition-colors ${schisciaEnabled ? "bg-primary/10" : "bg-card"}`}>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Cena di oggi → pranzo di domani</p>
                    </div>
                  </div>
