    ...
    <div className="mt-12 grid gap-4 md:grid-cols-3">
      {insights.map((t) => (
        <div key={t} className="rounded-2xl border border-border/60 bg-card p-6 text-center">
          <p className="text-base md:text-lg text-foreground leading-relaxed">{t}</p>
        </div>
      ))}
    </div>
    <blockquote className="mt-12 text-center text-xl md:text-2xl italic text-foreground max-w-3xl mx-auto">
      Gli utenti non cercano principalmente risparmio.
      <span className="block mt-2 font-semibold">Cercano semplicità.</span>
    </blockquote>
  </SlideShell>
);
